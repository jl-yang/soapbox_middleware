...
class Middleware(object):
    ...
    def connect(self):
        ...
        self.connection = pika.SelectConnection(pika.URLParameters(self._url),
...
def on_channel_open(new_channel):
    ...
    result = channel.queue_declare(queue=queue_name, durable=True, exclusive=True, auto_delete=False, callback=on_queue_declared)
...
def start_middleware():
    middleware = Middleware("localhost")
    try:
        middleware.run()
    except K
